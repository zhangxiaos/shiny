...
var mSlide = function(options) {
  this.options    = options || {};
  this.wrap       = options.wrap;
  this.type       = options.type || 'X';
  this.onSlideEnd = this.options.onSlideEnd || function(){};
...
mSlide.prototype = {
  ...
  _touchmove: function(e) {
    var touches = e.touches[0],
        X       = touches.pageX - this.pageX,
        Y       = touches.pageY - this.pageY,
    ...
    if (this.flag === this.type) {
      ...
      switch (this.type) {
        case 'X':
          this.move = X;
          this._setX(curEl, X);
          next && (this._setX(next, X + this.width));
          prev && (this._setX(prev, X - this.width));
          break;
        case 'Y':
          this.move = Y;
          this._setY(curEl, Y);
          next && (this._setY(next, Y + this.height));
          prev && (this._setY(prev, Y - this.height));
          break;
      }
    }
  },
